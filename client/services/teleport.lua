...
function CharacterTeleport:IsBusy()
    return teleporting
end

function CharacterTeleport:FindSurface(x, y, fallbackZ, options)
...
function CharacterTeleport:ToCoords(coords, options)
    options = type(options) == 'table' and options or {}
...
function CharacterTeleport:ToWaypoint(options)
...
exports('TeleportToCoords', function(coords, options) return CharacterTeleport:ToCoords(coords, options) end)
...
RegisterCommand('CharacterUtilityOwnershipSmokeTest', function()
    local owned = type(CharacterTeleport) == 'table'
        and type(CharacterTeleport.ToCoords) == 'function'
        and type(CharacterTeleport.FindSurface) == 'function'
    local legacyCalled = pcall(function()
        return exports['feather-core']:TeleportToCoords({ x = 0.0, y = 0.0, z = 0.0 }, { fade = false })
    end)
    print(('[CharacterUtilityOwnershipSmokeTest] character teleport owned  %s'):format(owned and 'PASS' or 'FAIL'))
    print(('[CharacterUtilityOwnershipSmokeTest] core teleport absent      %s'):format(not legacyCalled and 'PASS' or 'FAIL'))
    print(('[CharacterUtilityOwnershipSmokeTest] done %d/2 passed'):format(
        (owned and 1 or 0) + (not legacyCalled and 1 or 0)))
end, false)
