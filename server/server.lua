...
RegisterServerEvent('feather-character:SendDetailsToDB', function(args)
    local src = source
    local activeuser = Feather.User.GetUserBySrc(src)
    Feather.Character.CreateCharacter(activeuser.id, 1, args.firstname, args.lastname, args.dob, Config.defaults.money,
    ...
    for k, v in ipairs(GetPlayerIdentifiers(_source)) do
        if string.match(v, "license:") then
            ...
            if license[1] then
                ...
                print(userid)
                exports.ghmattimysql:execute("SELECT * FROM `characters` WHERE user_id = @userid",{ ['userid'] = userid },
                    function(result)
                        if result[1] then
                            local allchars = Feather.Character.GetAvailableCharactersFromDB(_source)
